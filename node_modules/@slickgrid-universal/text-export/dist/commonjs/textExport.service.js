...
class TextExportService {
    ...
    exportToFile(options) {
        ...
        return new Promise(resolve => {
            ...
            this._exportOptions = common_1.deepCopy({ ...DEFAULT_EXPORT_OPTIONS, ...this._gridOptions.exportOptions, ...this._gridOptions.textExportOptions, ...options });
    ...
    startDownloadFile(options) {
        ...
        const csvContent = common_1.htmlEntityDecode(options.content);
    ...
    getDataOutput() {
        ...
        if (!groupByColumnHeader && this._gridOptions.enableTranslate && this._translaterService && this._translaterService.translate && this._translaterService.getCurrentLanguage && this._translaterService.getCurrentLanguage()) {
            groupByColumnHeader = this._translaterService.translate(`${common_1.getTranslationPrefix(this._gridOptions)}GROUP_BY`);
        }
        ...
        if (columns && Array.isArray(columns)) {
            ...
            columns.forEach((columnDef) => {
                ...
                if ((columnDef.nameKey || columnDef.nameKey) && this._gridOptions.enableTranslate && this._translaterService && this._translaterService.translate && this._translaterService.getCurrentLanguage && this._translaterService.getCurrentLanguage()) {
                    ...
                    headerTitle = columnDef.name || common_1.titleCase(columnDef.field);
                }
                const skippedField = columnDef.excludeFromExport || false;
                // if column width is 0px, then we consider that field as a hidden field and should not be part of the export
                if ((columnDef.width === undefined || columnDef.width > 0) && !skippedField) {
    ...
    readRegularRowData(columns, row, itemObj) {
        ...
        for (let col = 0, ln = columns.length; col < ln; col++) {
            ...
            if ((prevColspan === '*' && col > 0) || (prevColspan > 1 && columnDef.id !== colspanColumnId)) {
                ...
                let itemData = common_1.exportWithFormatterWhenDefined(row, col, columnDef, itemObj, this._grid, this._exportOptions);
                // does the user want to sanitize the output data (remove HTML tags)?
                if (columnDef.sanitizeDataExport || this._exportOptions.sanitizeDataExport) {
                    itemData = common_1.sanitizeHtmlToText(itemData);
                }
    ...
    readGroupedTitleRow(itemObj) {
        let groupName = common_1.sanitizeHtmlToText(itemObj.title);
        const exportQuoteWrapper = this._exportQuoteWrapper;
        groupName = common_1.addWhiteSpaces(5 * itemObj.level) + groupName;
