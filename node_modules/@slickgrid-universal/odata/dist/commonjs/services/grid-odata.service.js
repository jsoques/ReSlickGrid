    ...
    updateFilters(columnFilters, isUpdatedByPresetOrDynamically) {
        ...
        for (const columnId in columnFilters) {
            if (columnFilters.hasOwnProperty(columnId)) {
                ...
                const lastValueChar = (!!matches) ? matches[3] : (operator === '*z' || operator === common_1.OperatorType.endsWith) ? '*' : '';
                ...
                if (!operator) {
                    operator = common_1.mapOperatorByFieldType(fieldType);
                }
                ...
                if (bypassOdataQuery) {
                    ...
                    if (this._odataService.options.caseType === common_1.CaseType.pascalCase) {
                        fieldName = common_1.titleCase(fieldName || '');
                    }
    ...
    updateSorters(sortColumns, presetSorters) {
                ...
                if (sortColumns) {
                    for (const columnDef of sortColumns) {
                        if (columnDef.sortCol) {
                            ...
                            if (this._odataService.options.caseType === common_1.CaseType.pascalCase) {
                                fieldName = common_1.titleCase(fieldName);
                                columnFieldName = common_1.titleCase(columnFieldName);
                                queryField = common_1.titleCase(queryField);
                            }
                            if (columnFieldName !== '') {
                                currentSorters.push({
        ...
        const csvString = odataSorters.map((sorter) => {
            ...
            if (sorter && sorter.field) {
                const sortField = (this._odataService.options.caseType === common_1.CaseType.pascalCase) ? common_1.titleCase(sorter.field) : sorter.field;
    ...
    normalizeSearchValue(fieldType, searchValue, version) {
        switch (fieldType) {
            case common_1.FieldType.date:
                searchValue = common_1.parseUtcDate(searchValue, true);
