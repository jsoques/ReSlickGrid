...
class FilterService {
    ...
    disposeColumnFilters() {
        ...
        if (Array.isArray(this._filtersMetadata)) {
            this._filtersMetadata.forEach(filter => {
                if (filter === null || filter === void 0 ? void 0 : filter.destroy) {
                    filter.destroy();
                }
            });
        }
    ...
    bindBackendOnFilter(grid) {
        ...
        this._eventHandler.subscribe(onHeaderRowCellRenderedHandler, (_e, args) => {
        ...
        if (this._onSearchChange) {
            ...
            this._eventHandler.subscribe(onSearchChangeHandler, this.onBackendFilterChange.bind(this));
        }
    }
    /**
    ...
    async clearFilters(triggerChange = true) {
        ...
        if (backendApi && triggerChange) {
            ...
            (_f = this.backendUtilities) === null || _f === void 0 ? void 0 : _f.executeBackendCallback(backendApi, query, callbackArgs, new Date(), totalItems, this.emitFilterChanged.bind(this));
        }
        ...
        if (isGridWithTreeData && this._gridOptions && this._gridOptions.treeDataOptions) {
            ...
            if (typeof columnFilters === 'object') {
                for (const columnId of Object.keys(columnFilters)) {
                    ...
                    let parsedSearchTerms = columnFilter === null || columnFilter === void 0 ? void 0 : columnFilter.parsedSearchTerms; // parsed term could a single value or an array of values
                    ...
                    if (parsedSearchTerms === undefined) {
                        parsedSearchTerms = index_1.getParsedSearchTermsByFieldType(columnFilter.searchTerms, columnFilter.columnDef.type || index_2.FieldType.string); // parsed term could a single value or an array of values
                        if (parsedSearchTerms !== undefined) {
    ...
    parseFormInputFilterConditions(inputSearchTerms, columnFilter) {
        var _a, _b, _c;
        const searchValues = utilities_1.deepCopy(inputSearchTerms) || [];
    ...
    preProcessFilterConditionOnDataContext(item, columnFilter, grid) {
        ...
        if ((queryFieldName === null || queryFieldName === void 0 ? void 0 : queryFieldName.indexOf('.')) >= 0) {
            cellValue = utilities_1.getDescendantProperty(item, queryFieldName);
        }
    ...
    preFilterTreeData(inputItems, columnFilters) {
        ...
        const dataViewIdIdentifier = (_f = (_d = this._gridOptions) === null || _d === void 0 ? void 0 : _d.datasetIdPropertyName) !== null && _f !== void 0 ? _f : 'id';
        const treeDataToggledItems = (_h = (_g = this._gridOptions.presets) === null || _g === void 0 ? void 0 : _g.treeData) === null || _h === void 0 ? void 0 : _h.toggledItems;
        const treeObj = {};
        const filteredChildrenAndParents = [];
        if (Array.isArray(inputItems)) {
            for (let i = 0; i < inputItems.length; i++) {
                treeObj[inputItems[i][dataViewIdIdentifier]] = inputItems[i];
                ...
                delete treeObj[inputItems[i][dataViewIdIdentifier]].__used;
            }
            ...
            for (const columnId of Object.keys(columnFilters)) {
                const columnFilter = columnFilters[columnId];
                const searchValues = (columnFilter === null || columnFilter === void 0 ? void 0 : columnFilter.searchTerms) ? utilities_1.deepCopy(columnFilter.searchTerms) : [];
                const inputSearchConditions = this.parseFormInputFilterConditions(searchValues, columnFilter);
                const columnDef = columnFilter.columnDef;
                const fieldType = (_l = (_k = (_j = columnDef === null || columnDef === void 0 ? void 0 : columnDef.filter) === null || _j === void 0 ? void 0 : _j.type) !== null && _k !== void 0 ? _k : columnDef === null || columnDef === void 0 ? void 0 : columnDef.type) !== null && _l !== void 0 ? _l : index_2.FieldType.string;
                const parsedSearchTerms = index_1.getParsedSearchTermsByFieldType(inputSearchConditions.searchTerms, fieldType); // parsed term could a single value or an array of values
                if (parsedSearchTerms !== undefined) {
                    columnFilter.parsedSearchTerms = parsedSearchTerms;
                }
            }
            ...
            for (let i = 0; i < inputItems.length; i++) {
                ...
                for (const columnId of Object.keys(columnFilters)) {
                    ...
                    if (conditionOptionResult) {
                        const parsedSearchTerms = columnFilter === null || columnFilter === void 0 ? void 0 : columnFilter.parsedSearchTerms; // parsed term could a single value or an array of values
                ...
                if (matchFilter) {
                    ...
                    let parent = (_m = treeObj[item[parentPropName]]) !== null && _m !== void 0 ? _m : false;
                    // if there are any presets of collapsed parents, let's processed them
                    const presetToggleShouldBeCollapsed = !((_o = this._gridOptions.treeDataOptions) === null || _o === void 0 ? void 0 : _o.initiallyCollapsed);
                    ...
                    while (parent) {
                        ...
                        (_p = parent.__used) !== null && _p !== void 0 ? _p : filteredChildrenAndParents.splice(len, 0, parent[dataViewIdIdentifier]);
                        ...
                        parent = (_q = treeObj[parent[parentPropName]]) !== null && _q !== void 0 ? _q : false;
                    }
                }
            }
        }
    ...
    async onBackendFilterChange(event, args) {
        ...
        if (isTriggeringQueryEvent) {
            ...
            (_d = this.backendUtilities) === null || _d === void 0 ? void 0 : _d.executeBackendCallback(backendApi, query, args, startTime, totalItems, this.emitFilterChanged.bind(this), this.httpCancelRequests$);
        }
    }
    /**
        ...
        if (args) {
            ...
            if (columnDef && columnId) {
                if (!hasSearchTerms || termsCount === 0 || (termsCount === 1 && Array.isArray(searchTerms) && emptySearchTermReturnAllValues && searchTerms[0] === '')) {
                    ...
                    colFilter.operator = operator || inputSearchConditions.operator || utilities_1.mapOperatorByFieldType(fieldType);
                    parsedSearchTerms = index_1.getParsedSearchTermsByFieldType(inputSearchConditions.searchTerms, fieldType);
                    if (parsedSearchTerms !== undefined) {
            ...
            const eventKey = (_h = event) === null || _h === void 0 ? void 0 : _h.key;
            const eventKeyCode = (_j = event) === null || _j === void 0 ? void 0 : _j.keyCode;
            if (this._onSearchChange && (args.forceOnSearchChangeEvent || eventKey === 'Enter' || eventKeyCode === index_2.KeyCode.ENTER || !lite_1.dequal(oldColumnFilters, this._columnFilters))) {
                this._onSearchChange.notify({
                    ...
                    columnDef,
                    columnFilters: this._columnFilters,
                    operator: operator || utilities_1.mapOperatorByFieldType(fieldType),
                    searchTerms,
                    parsedSearchTerms,
                    grid: this._grid
                }, eventData);
    ...
    disableAllFilteringCommands(isDisabling = true) {
        ...
        if ((_b = (_a = this._gridOptions) === null || _a === void 0 ? void 0 : _a.gridMenu) === null || _b === void 0 ? void 0 : _b.customItems) {
    ...
    updateColumnFilters(searchTerms, columnDef, operator) {
        var _a, _b, _c;
        const fieldType = (_c = (_b = (_a = columnDef.filter) === null || _a === void 0 ? void 0 : _a.type) !== null && _b !== void 0 ? _b : columnDef.type) !== null && _c !== void 0 ? _c : index_2.FieldType.string;
        const parsedSearchTerms = index_1.getParsedSearchTermsByFieldType(searchTerms, fieldType); // parsed term could a single value or an array of values
