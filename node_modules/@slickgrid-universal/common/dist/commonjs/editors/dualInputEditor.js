...
class DualInputEditor {
    ...
    createInput(position) {
        ...
        input.setAttribute('aria-label', (_e = (_d = this.columnEditor) === null || _d === void 0 ? void 0 : _d.ariaLabel) !== null && _e !== void 0 ? _e : `${utilities_1.toSentenceCase(columnId + '')} Input Editor`);
    ...
    getValues() {
        ...
        utilities_1.setDeepValue(obj, this._leftFieldName, resultLeftValue);
        utilities_1.setDeepValue(obj, this._rightFieldName, resultRightValue);
        return obj;
    }
    setValues(values) {
    ...
    applyValueByPosition(item, state, position) {
        ...
        if (fieldName !== undefined) {
            ...
            const stateValue = isComplexObject ? utilities_1.getDescendantProperty(state, fieldNameToUse) : state[fieldName];
            const validation = this.validate(null, { position, inputValue: stateValue });
            // set the new value to the item datacontext
            if (isComplexObject) {
                const newValueFromComplex = utilities_1.getDescendantProperty(state, fieldNameToUse);
    ...
    loadValueByPosition(item, position) {
        ...
        if (item && fieldName !== undefined) {
            ...
            const itemValue = (isComplexObject) ? utilities_1.getDescendantProperty(item, fieldName) : (item.hasOwnProperty(fieldName) ? item[fieldName] : '');
    ...
    serializeValue() {
        ...
        utilities_1.setDeepValue(obj, this._leftFieldName, leftValue);
        utilities_1.setDeepValue(obj, this._rightFieldName, rightValue);
        return obj;
    }
    ...
    validateByPosition(position, inputValue) {
        ...
        switch (positionEditorParams.type) {
            case 'float':
                return editorValidators_1.floatValidator(currentVal, {
            ...
            case 'integer':
                return editorValidators_1.integerValidator(currentVal, {
                    ...baseValidatorOptions,
                    minValue: positionEditorParams.minValue,
                    maxValue: positionEditorParams.maxValue,
                    operatorConditionalType: positionEditorParams.operatorConditionalType,
            ...
            case 'password':
            default:
                return editorValidators_1.textValidator(currentVal, baseValidatorOptions);
        }
    }
    /** when it's a Composite Editor, we'll check if the Editor is editable (by checking onBeforeEditCell) and if not Editable we'll disable the Editor */
