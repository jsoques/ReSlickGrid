import { CellMenu, ColumnEditor, ColumnFilter, EditorValidator, Formatter, Grouping, GroupTotalsFormatter, HeaderButtonItem, MenuCommandItem, OnEventArgs, SlickEventData, SortComparer } from './index';
import { FieldType } from '../enums/fieldType.enum';
declare type PathsToStringProps<T> = T extends string | number | boolean | Date ? [] : {
    [K in Extract<keyof T, string>]: [K, ...PathsToStringProps<T[K]>];
}[Extract<keyof T, string>];
declare type Join<T extends any[], D extends string> = T extends [] ? never : T extends [infer F] ? F : T extends [infer F, ...infer R] ? F extends string ? string extends F ? string : `${F}${D}${Join<R, D>}` : never : string;
